...
function paint(r = null) {
    ...
    if (radius != null) {
        let [left, bottom] = convertCleanToCanvasCoordinates(-4, -5, radius, centerX, centerY, pointDistance);
        ...
        canvas.click(function () {
            alert("Задайте значение R и выполните проверку.");
        });
    }
    ...
    context.arc(centerX, centerY, pointDistance, Math.PI / 2, 0, true);
    context.moveTo(centerX, centerY);
    ...
    $(".history-x").each(function () {
        historyX.push(+$(this).text().substring(0, 15))
    });

    let historyY = [];
    $(".history-y").each(function () {
        historyY.push(+$(this).text().substring(0, 15))
    });
